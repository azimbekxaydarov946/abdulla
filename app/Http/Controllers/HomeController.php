...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $counts =  DB::table('user_days')->rightJoin('days', 'user_days.day_id', '=', 'days.id')->select(DB::raw('days.day as day,count(user_days.user_id) as count_users'))->where('days.day', 'like', '%' . $filter . '%')->groupBy('days.id')->groupBy('days.day')->orderBy('days.id')->get();


        $todays = $this->UserDayService->get()->where('day_id',$now )->get();

        return view('page.home', compact('days', 'users', 'user_days',  'todays','month','counts', 'payments',  'sum','ray'));
    }
}
